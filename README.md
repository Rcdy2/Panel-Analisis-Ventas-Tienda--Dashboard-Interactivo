...
- **Filtro Temporal:** Un segmentador de fechas que permite visualizar el rendimiento para "Todos los periodos" o filtrar por meses específicos del año 2026, facilitando el análisis de tendencias.
...
- **Alta Concentración de Ingresos:** Las Tiendas A, B y C generan la gran mayoría de los ingresos (S/ 1.14M+). **Tienda D y E presentan una contribución mínima o nula**, lo que representa una oportunidad crítica de análisis para entender las causas: bajo tráfico, falta de stock, mala ubicación o nula operación comercial.
...
3. **Visualización y Creación del Dashboard:** Se diseñó la interfaz del dashboard conectándola a las tablas dinámicas. Se insertaron **múltiples segmentadores de datos (escala de tiempo, categoría, tienda, vendedor)** para hacer el reporte altamente interactivo. Se utilizaron formato condicional y barras de datos para mejorar la legibilidad de las métricas.
4. **Análisis y Validación:** Se revisaron los datos para asegurar la coherencia entre las diferentes vistas del dashboard (ej. que la suma de ventas por tienda coincida con el total general) y que los filtros interactivos funcionen correctamente.
...
- **Pensamiento Analítico:** Identificación de anomalías (como el valor extraño de unidades en Tienda A) y puntos de mejora a partir de los datos presentados.

## Funciones y Técnicas Utilizadas
- **Manipulación de Datos:** Funciones como `SUMAR.SI.CONJUNTO`, `CONTAR.SI.CONJUNTO` y `BUSCARV` (si se usó para enriquecer datos).
...
2. Navega a la carpeta del proyecto y abre el archivo dashboard reporte de ventas productos tecnologicos.xlsx con Microsoft Excel (2016 o superior para una mejor compatibilidad con segmentadores y escala de tiempo).
